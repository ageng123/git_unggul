...
class BarangRepository implements IMasterDataRepository{
    public function __construct(private Barang $model){}
    public function model(int $id = null){
        if($id != null && !empty($id)){
            $find = $this->model->find($id)->load(['history_barang_keluar']);
            if(!$find){
