...
class PelangganRepository implements IMasterDataRepository{
    public function __construct(private Pelanggan $model){}
    public function model(int $id = null){
        if($id != null && !empty($id)){
            $find = $this->model->find($id)->load(['history_nota']);
            if(!$find){
