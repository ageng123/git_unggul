...
class ItemPenjualanRepository implements IMasterDataRepository{
    public function __construct(private ItemPenjualan $model){}
    ...
    public function update($formdata = null, int $id = null){
        ...
        $data = $this->model($id);
        $update = $data->update($formdata->toArray());
        if(!$update){
